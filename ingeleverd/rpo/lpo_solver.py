...
def gen_sub_name(s: Term, t: Term) -> str:
    """
    Given two terms s and t, generates the string [s >sub t].
    """
    if not isinstance(s, Term) and isinstance(t, Term):
        raise TypeError(f"The arguments is of type {type(s)} and {type(t)} "
                        f"while they should be of type <class tm.Term>")
    return "[" + tm.to_string(s) + ">sub" + tm.to_string(t) + "]"

def gen_copy_name(s: Term, t: Term) -> str:
    """
    Given two terms s and t, generates the string [s >copy t].
    """
    if not isinstance(s, Term) and isinstance(t, Term):
        raise TypeError(f"The arguments is of type {type(s)} and {type(t)} "
                        f"while they should be of type <class tm.Term>")
    return "[" + tm.to_string(s) + ">copy" + tm.to_string(t) + "]"

def gen_lex_name(s: Term, t: Term) -> str:
    """
    Given two terms s and t, generates the string [s >lex t].
    """
    if not isinstance(s, Term) and isinstance(t, Term):
        raise TypeError(f"The arguments is of type {type(s)} and {type(t)} "
                        f"while they should be of type <class tm.Term>")
    return "[" + tm.to_string(s) + ">lex" + tm.to_string(t) + "]"
...
# def gen_z3_sub(s: Term, t: Term) -> None:
#     z3_sub[(s, t)] = z3.Bool(gen_sub_name(s, t))

# def gen_z3_copy(s: Term, t: Term) -> None:
#     z3_copy[(s, t)] = z3.Bool(gen_copy_name(s, t))

# def gen_z3_lex(s: Term, t: Term) -> None:
#     z3_lex[(s, t)] = z3.Bool(gen_lex_name(s, t))
...
def gen_z3_prec_vars(fs: list[FnSym]) -> None:
    ...
    g = fs[2]
    h = fs[4]
    solver.add(z3.Not(z3_prec[f] == z3_prec[a] == z3_prec[h] == z3_prec[c] > z3_prec[d] == z3_prec[b] > z3_prec[g]))
...
def _gen_z3_ctrs(u: Term, v: Term):
    ...
    match (u, v):
        ...
        case (FnApp((f, args_u)), FnApp((g, args_v))) if f == g and len(args_u) == len(args_v) > 0:
            # Sub+multiset rule
            # print("f", f, "args_u", args_u)
            # print("g", g, "args_v", args_v)
            eq_vars = [z3.Bool(f"EQ_{next(counter)}_{n}") for n in range(len(args_u))]
            # print(eq_vars)
            phi_vars = [z3.Int(f"phi_{next(counter)}_{m}") for m in range(len(args_v))]
            # print(phi_vars)

            or_exprs = z3.Or(list(map(lambda x: z3_gte[(x, v)], args_u))) # sub rule

            eq_false_exprs = [z3.Implies(z3.And(phi_vars[m] == n, z3.Not(eq_vars[n])), z3_gt[(args_u[n], args_v[m])]) 
             for n in range(len(args_u)) 
             for m in range(len(args_v))]
            # print("eq_false_exprs")
            # solver.add(eq_false_exprs)
            eq_true_exprs = [z3.Implies(z3.And(phi_vars[m] == n, eq_vars[n]), tm.term_eq(args_u[n], args_v[m])) 
             for n in range(len(args_u)) 
             for m in range(len(args_v))]
            # print("eq_true_exprs")
            # solver.add(eq_true_exprs)
            eq_at_least_one_false = z3.Sum([z3.If(z3.Not(eq_vars[n]),1,0) for n in range(len(args_u))]) == 1
            # print("eq_at_least_one_false")
            # solver.add(eq_at_least_one_false)
            and_exprs = z3.And(z3.And(eq_false_exprs), z3.And(eq_true_exprs), z3.And(eq_at_least_one_false))
            ...
            # This seems like complete malarkey? This constraint is never generated
            # [u > v] -> (
            #              (\/_i [u_i >= v]) \/
            #              ([u_i > v_i] /\
            #              (/\_{j = i + 1}^n [u > v_j])) )
        ...
        case (FnApp((f, args_u)), FnApp((g, args_v))) if (not f == g):
            # Copy rule
            or_exprs = z3.Or(list(map(lambda x: z3_gte[(x, v)], args_u))) # sub rule
            and_exprs = z3.And(( # copy rule
                    [z3_prec[f] >= z3_prec[g]] + # Changing to quasi-ordering by allowing geq
...
def print_inequalities(model, rules):
    ...
    for pairindex in range(len(pairs)):
        ...
        if leftroot == rightroot and len(leftargs) == len(rightargs) and leftargs != rightargs:
            ...
            # reasons = [ str(pairs.index(p)) if p in pairs else "ERROR mul reason " + str(p[0]) + ">" + str(p[1]) for p in needed]
            # print(", ".join(reasons))
            continue
        if rightroot != None and model.evaluate(z3_prec[leftroot]).as_long() >= model.evaluate(z3_prec[rightroot]).as_long():
            ...
            reasons = [ str(pairs.index(p)) if p in pairs else "ERROR copy reason " + str(p[0]) + ">" + str(p[1]) for p in needed]
            print(", ".join(reasons))
            continue
